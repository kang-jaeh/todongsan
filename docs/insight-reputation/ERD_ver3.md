# docs/insight-reputation/ERD_ver3.md

> Insight-Reputation Service의 데이터베이스 설계 문서  
> v3: public_data_snapshot 테이블 추가 (공공 API 배치 적재), insight_report status/failed_reason 컬럼 추가, AI 데이터 흐름 개정

---

## 변경 내역 (ERD_ver2.md → ERD_ver3.md)

| 테이블 | 변경 내용 |
|---|---|
| `public_data_snapshot` | 신규 추가. 공공 API 배치 수집 데이터 적재 (R-ONE, 국토교통부 등 멀티 소스 지원) |
| `insight_report` | `status VARCHAR(20)`, `failed_reason TEXT` 컬럼 추가. AI 분석 상태 추적 |
...
> **status 전이 흐름**  
> `PENDING` (트리거 시 INSERT) → `PROCESSING` (Claude API 호출 시작) → `DONE` (저장 완료) / `FAILED` (오류 발생 시 failed_reason 기록)
...
CREATE TABLE public_data_snapshot (
    ...
    source_region_id    VARCHAR(50),                -- 소스별 지역 고유 코드. REB: cls_id, MOLIT: 법정동코드 앞 5자리
    raw_data            JSON            NOT NULL,   -- API 원본 응답 전체. 소스별 구조 상이
    collected_at        DATETIME        NOT NULL,   -- 수집 시점
    created_at          DATETIME        NOT NULL,
    PRIMARY KEY (id),
    UNIQUE KEY uq_snapshot (source, data_type, reference_date, source_region_id), -- 배치 재실행 시 ON DUPLICATE KEY UPDATE
...
>   "dtacycle_cd": "WK",
>   "wrttime_idtfr_id": "202520",
>   "cls_id": 50043,
>   "cls_nm": "종로구",
>   "cls_fullnm": "서울>강북지역>도심권>종로구",
>   "itm_id": 10001,
>   "itm_nm": "지수",
>   "dta_val": 95.2273702404922,
>   "ui_nm": "지수"
> }
> ```
...
> **지역 필터링 쿼리 예시**
> ```sql
> -- 서울 전체 최신 주간 매매가격지수 조회
> SELECT JSON_EXTRACT(raw_data, '$.cls_fullnm') AS region,
>        JSON_EXTRACT(raw_data, '$.dta_val')    AS index_value
> FROM   public_data_snapshot
> WHERE  source = 'REB'
>   AND  data_type = 'PRICE_INDEX'
>   AND  reference_date = '2025-05-12'
>   AND  region_sido = '서울';
>
> -- 변동률 계산 (이번 주 vs 저번 주)
> SELECT this_week.source_region_id,
>        JSON_EXTRACT(this_week.raw_data, '$.cls_fullnm') AS region,
>        JSON_EXTRACT(this_week.raw_data, '$.dta_val') - JSON_EXTRACT(last_week.raw_data, '$.dta_val') AS diff
...
erDiagram
    ...
    public_data_snapshot {
        ...
        VARCHAR source_region_id "소스별 지역 고유 코드"
...
[AI 분석 트리거] (Battle 종료 / Market 정산)
  ...
  → public_data_snapshot 조회 (필요 지역/기간 필터링)
  → Battle/Market 원본 데이터 조회 (REST)
  → insight_report PROCESSING 업데이트
  → Claude API 호출 (수집 데이터 + 원본 데이터 프롬프트 조합)
  → insight_report DONE + summary 저장
  → (실패 시) insight_report FAILED + failed_reason 저장
  → Client에 리포트 제공
```
...
- `public_data_snapshot`: `(source, data_type, reference_date, source_region_id)` UNIQUE (배치 재실행 시 `ON DUPLICATE KEY UPDATE raw_data = VALUES(raw_data), collected_at = VALUES(collected_at)`)
...
- `FAILED` 상태 레코드는 스케줄러가 재시도 가능. 재시도 시 `PENDING`으로 리셋.
...
- 배치 실패 시 `insight_report`의 관련 레코드를 `FAILED`로 처리하지 않고 다음 배치 주기까지 `PENDING` 유지
